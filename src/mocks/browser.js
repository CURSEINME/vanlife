...
export const worker = setupWorker(
  rest.get("/api/vans", (req, res, ctx) => {
    return res(ctx.json(vansData.vans), ctx.delay(2000))
  }),

  rest.get("/api/vans/:id", (req, res, ctx) => {
    const id = req.params.id
    const requestedVan = vansData.vans.find(item => item.id === id)
    return res(ctx.json(requestedVan))
  }),

  rest.get("/api/host/vans", (req, res, ctx) => {
    return res(ctx.json(vansData.vans))
  }),

  rest.get("/api/host/vans/:id", (req, res, ctx) => {
    const id = req.params.id
    const requestedVan = vansData.vans.find(item => item.id === id)
    return res(ctx.json(requestedVan))
  }),
  ...
  rest.post("/api/login", (req, res, ctx) => {
    ...
    if(!founduser) {
      return res(
        ctx.status(403),
        ctx.json({ message: "No user with those credentials found!" })
      )
    }
